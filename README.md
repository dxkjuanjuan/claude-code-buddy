# Claude Code Buddy — 基于 clawd-on-desk 与 M5StickC Plus 的 Clawstick / Hardware Buddy for Claude Code

Claude Code Buddy 是一个基于 M5StickC Plus 开发板的硬件伴侣，通过 clawd-on-desk 软件与 BLE 连接到电脑，实时显示 Claude Code 的 16 种 clawd 表情，并支持在 M5 上审批权限请求。

Claude Code Buddy is an M5StickC Plus-based hardware companion that connects to your PC via clawd-on-desk and BLE, displays 16 clawd expressions from Claude Code in real time, and supports permission approval on the M5 device.

...
inline bool xferCommand(JsonDocument& doc) {
  ...
  if (strcmp(cmd, "species") == 0) {
    // L2 dropped ASCII species support (the buddy module was deleted).
    // Acknowledge silently so legacy sidecars expecting a reply don't
    // hang their queue. The NVS "species" key, if leftover from a
    // pre-L2 install, is no longer read on boot; reset() still clears
    // the whole "buddy" namespace.
    _xAck("species", true);
    return true;
  }
  ...
  if (strcmp(cmd, "status") == 0) {
    ...
    // Plan §3.3 method B fields: dropped velocity ring, added shakes /
    // mood / energy. Sidecar consumers that still read "vel" will see it
    // missing — the L2-era field never carried useful info to the desktop
    // anyway (the velocity-based mood formula was the fork bug we're
    // replacing).
    char b[360];
    int len = snprintf(b, sizeof(b),
      ...
      "\"stats\":{\"appr\":%u,\"deny\":%u,\"nap\":%lu,\"shakes\":%u,"
                  "\"lvl\":%u,\"mood\":%u,\"energy\":%u,\"rtc\":%s}"
      "}}\n",
      ...
      statsApprovals(), statsDenials(),
      (unsigned long)statsNapSecondsTotal(), statsTotalShakes(),
      statsLevel(), statsMoodTier(), statsEnergyTier(),
      statsRtcSynced() ? "true" : "false"
    );
    Serial.write(b, len);
    bleWrite((const uint8_t*)b, len);
    return true;
...
#if CLAWSTICK_BENCH_HEAP
// L0a-2 integrated bench: publish a compact heap snapshot every second.
// Goes to both Serial and BLE so sidecar log captures it regardless of
// transport. Format is intentionally distinct from the "status" command
// (sidecar can filter on bench=heap to separate bench rows from regular
// status responses).
//
// nap / screenOff fields hint at why frames may have stalled. After L2-1b,
// napping no longer skips sprite render (plan §2.3) so the nap field is
// expected to be true with non-zero frame deltas; only screen-off should
// correlate with zero-fps windows.
extern bool napping;
extern bool screenOff;
inline void xferPublishBenchHeap() {
  uint32_t heap8       = heap_caps_get_free_size(MALLOC_CAP_8BIT);
  ...
  uint32_t largestInt  = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  // Fragmentation pct: 0 = perfectly contiguous, 100 = totally fragmented.
  uint32_t fragPct     = heap8 > 0 ? (uint32_t)(100ULL - (uint64_t)largest8 * 100ULL / heap8) : 0;
  uint32_t frames      = characterFramesPlayed();
  // ack:"bench_heap" lets the bleak sidecar (tools/backends/bleak_backend.py:287)
  // route this through its generic command-forwarding path. Without an ack
  // or cmd field the sidecar warns and drops the line. bench:"heap" is kept
  // for easy log grepping; both fields are honored.
  char b[256];
  int len = snprintf(b, sizeof(b),
    "{\"ack\":\"bench_heap\",\"bench\":\"heap\",\"up\":%lu,"
    "\"heap_8bit\":%lu,\"largest_8bit\":%lu,"
    "\"heap_internal\":%lu,\"largest_internal\":%lu,"
    "\"frag_pct\":%lu,\"frames\":%lu,\"sec\":%s,"
    "\"nap\":%s,\"so\":%s}\n",
    (unsigned long)(millis() / 1000),
    (unsigned long)heap8, (unsigned long)largest8,
    (unsigned long)heapInt, (unsigned long)largestInt,
    (unsigned long)fragPct, (unsigned long)frames,

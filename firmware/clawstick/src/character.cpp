...
// L2-2: 5-state schema (plan §2.2). Each state is one looped GIF.
static const char* STATE_NAMES[] = {
  "idle", "working", "thinking", "dizzy", "error"
};
static const uint8_t N_STATES = 5;

// Maps the firmware's PersonaState (P_SLEEP=0..P_ERROR=7, defined in
// main.cpp) onto the 5-state Clawd asset set. Sleep is simplified to
// idle + dim (plan §2.3); celebrate / heart fall back to idle since the
// asset set has no dedicated celebration GIF. P_ERROR is wired in L4-1
// (plan §2.2): BLE transport degraded mid-session lights the error GIF
// so a missed approval isn't masked by an idle-looking screen.
//
// Index 8 (out-of-range) maps to idle as a safety fallback.
static const uint8_t PERSONA_TO_STATE[9] = {
  0,  // P_SLEEP     -> idle
  0,  // P_IDLE      -> idle
  1,  // P_BUSY      -> working
...
void characterSetState(uint8_t persona) {
  if (!loaded) return;
  // Caller passes a PersonaState (0..7) from main.cpp; map to 5-state.
  uint8_t s = (persona < 9) ? PERSONA_TO_STATE[persona] : 0;

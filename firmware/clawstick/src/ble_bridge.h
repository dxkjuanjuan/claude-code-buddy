...
// Stop/start discoverability without tearing down the BLE stack.
void bleSetEnabled(bool enabled);
bool bleEnabled();
bool bleAdvertising();
// While externally powered, keep BLE discoverable so the desktop can reconnect
// without a button press. Battery mode keeps the short advertising window.
void bleSetExternalPower(bool powered);
// Re-open the short discoverability window after a wake/button press.
void bleWakeAdvertising();
// Time-based housekeeping for discoverability timeouts.
void blePoll();
bool bleConnected();

...
enum PersonaState { P_SLEEP, P_IDLE, P_BUSY, P_ATTENTION, P_CELEBRATE, P_DIZZY, P_HEART, P_ERROR };
const char* stateNames[] = { "sleep", "idle", "busy", "attention", "celebrate", "dizzy", "heart", "error" };
...
PersonaState derive(const TamaState& s) {
  ...
  // L4-3 #5: user intentionally toggled BT off via Settings — treat as
  // P_SLEEP (no companion) rather than P_ERROR (degraded link). Without
  // this short-circuit a paired device going BT-off mid-session shows
  // the error GIF, which is indistinguishable UX-wise from a transport
  // failure. ble_bridge.cpp:171 also clears `secure` here, so this is
  // the policy layer separating "intentional off" from "lost link".
  if (!settings().bt) return P_SLEEP;
  // Plan §2.2 / §3.5.1: a secure session that's gone insecure is an
  // error state, not just "asleep" — the user paired the device, then
  // lost it. Surface visually so a missed approval isn't silently
  // swallowed (P_SLEEP looks identical to "no companion ever connected").
